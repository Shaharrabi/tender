/**
 * Nudge system — contextual prompts, reminders, and motivational messages
 * that guide the user through their relationship growth journey.
 *
 * Nudges are surfaced based on the user's current state (assessment progress,
...
/**
 * Returns an array of contextually relevant nudges based on the user's
 * current state. Nudges are sorted by priority (highest first).
 *
...
export function getNudges(
  ...
  nudges.push({
    id: `nudge-motivation-${quoteIndex}`,
    type: 'motivation',
    title: quote.title,
    body: quote.body,
    icon: '✨',
