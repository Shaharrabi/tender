...
function useBreatheOpacity(base = 0.85, range = 0.12, duration = 5000, delay = 0) {
...
export function IllustrationF20Onboarding({ width = 480, height, animated = true, style }: Props) {
  ...
  const bodyProps = useBreatheOpacity(0.85, 0.12, 5000, 0);
  const footProps = usePulseProps(0.50, 0.85, 3000, 0);
  const goalProps = usePulseProps(0.40, 0.80, 2000, 0);
  ...
  return (
    <View nativeID={containerId}>
      <Svg viewBox="0 0 480 340" width={width} height={resolvedHeight} style={style}>
        ...
        <AG id="onboard-body" {...(shouldAnimate ? { animatedProps: bodyProps } : {})}>
        ...
        {/* Foot step */}
        <AG id="onboard-foot" {...(shouldAnimate ? { animatedProps: footProps } : {})}>
          <Path d="M224 314 Q230 310 236 314" fill="none" stroke="#B5593A" strokeWidth="4" strokeLinecap="round"/>
        </AG>
        {/* Golden end-goal ring */}
        <AG id="onboard-goal" {...(shouldAnimate ? { animatedProps: goalProps } : {})}>
