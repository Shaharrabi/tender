...
/** Breathing opacity animation for figure groups on native */
function useBreatheOpacity(base = 0.80, range = 0.12, duration = 5000, delay = 0) {
...
export function IllustrationCommunityHero({ width = 520, height, animated = true, style }: Props) {
  ...
  const fig1Props = useBreatheOpacity(0.82, 0.12, 5000, 0);
  const fig2Props = useBreatheOpacity(0.78, 0.10, 4500, 1000);
  const fig3Props = useBreatheOpacity(0.85, 0.12, 5500, 2000);
  const fig4Props = useBreatheOpacity(0.75, 0.10, 6000, 500);
  const fig5Props = useBreatheOpacity(0.72, 0.10, 5000, 3000);
  const fig6Props = useBreatheOpacity(0.70, 0.10, 4000, 1500);
  const fig7Props = useBreatheOpacity(0.80, 0.12, 5000, 0);
  const bub1Props = usePulseProps(0.25, 0.55, 4000, 0);
  const bub2Props = usePulseProps(0.25, 0.55, 4000, 500);
  const bub3Props = usePulseProps(0.25, 0.55, 4000, 1000);
  const bub4Props = usePulseProps(0.25, 0.55, 4000, 1500);
  ...
  const F = shouldAnimate ? AnimatedG : G;
  const B = shouldAnimate ? AnimatedG : G;
  ...
  return (
    <View nativeID={containerId}>
      <Svg viewBox="0 0 520 380" width={width} height={resolvedHeight} style={style}>
        ...
        <F id="community-figure1" {...(shouldAnimate ? { animatedProps: fig1Props } : {})}>
        ...
        {/* Figure 2: sage */}
        <F id="community-figure2" {...(shouldAnimate ? { animatedProps: fig2Props } : {})}>
        ...
        {/* Figure 3: navy */}
        <F id="community-figure3" {...(shouldAnimate ? { animatedProps: fig3Props } : {})}>
        ...
        <F id="community-figure4" {...(shouldAnimate ? { animatedProps: fig4Props } : {})}>
        ...
        {/* Figure 5: blush/pink */}
        <F id="community-figure5" {...(shouldAnimate ? { animatedProps: fig5Props } : {})}>
        ...
        {/* Figure 6: plum */}
        <F id="community-figure6" {...(shouldAnimate ? { animatedProps: fig6Props } : {})}>
        ...
        <F id="community-figure7" {...(shouldAnimate ? { animatedProps: fig7Props } : {})}>
        ...
        {/* Attachment bubbles */}
        <B id="community-bubble1" {...(shouldAnimate ? { animatedProps: bub1Props } : {})}>
        ...
        <B id="community-bubble2" {...(shouldAnimate ? { animatedProps: bub2Props } : {})}>
        ...
        <B id="community-bubble3" {...(shouldAnimate ? { animatedProps: bub3Props } : {})}>
        ...
        <B id="community-bubble4" {...(shouldAnimate ? { animatedProps: bub4Props } : {})}>
