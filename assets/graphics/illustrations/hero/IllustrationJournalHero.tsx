...
function useBreatheOpacity(base = 0.85, range = 0.12, duration = 5500, delay = 0) {
...
export function IllustrationJournalHero({ width = 520, height, animated = true, style }: Props) {
  ...
  const bodyProps = useBreatheOpacity(0.85, 0.12, 5500, 0);
  const ringsProps = usePulseProps(0.35, 0.75, 2500, 0);
  const selfDotProps = usePulseProps(0.55, 0.95, 2000, 0);
  const sunProps = usePulseProps(0.35, 0.65, 6000, 0);
  const leafProps = usePulseProps(0.45, 0.80, 4000, 0);
  const heartProps = usePulseProps(0.40, 0.75, 3500, 1500);
  ...
  return (
    <View nativeID={containerId}>
      <Svg viewBox="0 0 520 380" width={width} height={resolvedHeight} style={style}>
        ...
        {/* Sun dot */}
        <AG id="journal-sun" {...(shouldAnimate ? { animatedProps: sunProps } : {})}>
        ...
        {/* BODY — terracotta */}
        <AG id="journal-body" {...(shouldAnimate ? { animatedProps: bodyProps } : {})}>
          <Path d="M190 350
          Q168 320 165 285
          Q162 252 175 224
          Q188 196 212 186
        ...
        <AG id="journal-rings" {...(shouldAnimate ? { animatedProps: ringsProps } : {})}>
        ...
        <AG id="journal-self-dot" {...(shouldAnimate ? { animatedProps: selfDotProps } : {})}>
          <Circle cx="222" cy="278" r="3.5" fill="#C8923A" opacity={0.85}/>
        </AG>
        {/* Floating journal symbols */}
        {/* Leaf */}
        <AG id="journal-leaf" {...(shouldAnimate ? { animatedProps: leafProps } : {})}>
        ...
        {/* Heart */}
        <AG id="journal-heart" {...(shouldAnimate ? { animatedProps: heartProps } : {})}>
