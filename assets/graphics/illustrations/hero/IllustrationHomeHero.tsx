 ...
 *           (same pattern as AttachFearful, IFSFirefighter, etc.)
 *
...
/** Breathing opacity for a body element on native */
function useBreatheOpacity(baseOpacity = 0.88, range = 0.12, duration = 5000, delay = 0) {
  ...
  useEffect(() => {
    ...
    opacity.value = withDelay(delay,
      withRepeat(
        withSequence(
          withTiming(baseOpacity - range, { duration: duration / 2, easing: Easing.inOut(Easing.ease) }),
          withTiming(baseOpacity, { duration: duration / 2, easing: Easing.inOut(Easing.ease) }),
        ),
        -1, false
      )
    );
...
/** Float opacity for leaf on native */
function useFloatOpacity(duration = 4000) {
  const opacity = useSharedValue(0.82);
  useEffect(() => {
    if (Platform.OS === 'web') return;
    opacity.value = withRepeat(
      withSequence(
        withTiming(0.55, { duration: duration / 2, easing: Easing.inOut(Easing.ease) }),
        withTiming(0.82, { duration: duration / 2, easing: Easing.inOut(Easing.ease) }),
      ),
      -1, false
    );
  }, []);
...
export function IllustrationHomeHero({ width = 400, height, animated = true, style }: Props) {
  ...
  // Native: per-element opacity animations via AnimatedG
  const leftBodyProps = useBreatheOpacity(0.88, 0.12, 5000, 0);
  const rightBodyProps = useBreatheOpacity(0.88, 0.12, 5000, 2500);
  const leafProps = useFloatOpacity(4000);
  const threadLeftProps = usePulseProps(0.18, 0.50, 3000, 0);
  const threadRightProps = usePulseProps(0.18, 0.50, 3000, 1500);
  ...
  // Choose G or AnimatedG based on platform + animated flag
  const LeftBody = (animated && isNative) ? AnimatedG : G;
  const RightBody = (animated && isNative) ? AnimatedG : G;
  const LeafGroup = (animated && isNative) ? AnimatedG : G;
  const ThreadLeftG = (animated && isNative) ? AnimatedG : G;
  const ThreadRightG = (animated && isNative) ? AnimatedG : G;
  ...
  return (
    <View nativeID={containerId}>
      <Svg viewBox="40 70 440 290" width={width} height={resolvedHeight} style={style}>
        ...
        <LeftBody id="home-left-body" {...(animated && isNative ? { animatedProps: leftBodyProps } : {})}>
        ...
        {/* RIGHT BODY — navy */}
        <RightBody id="home-right-body" {...(animated && isNative ? { animatedProps: rightBodyProps } : {})}>
        ...
        <LeafGroup id="home-leaf-group" {...(animated && isNative ? { animatedProps: leafProps } : {})}>
        ...
        {/* BREATH THREADS */}
        <ThreadLeftG id="home-thread-left" {...(animated && isNative ? { animatedProps: threadLeftProps } : {})}>
          <Line x1="144" y1="148" x2="200" y2="122" stroke="#2C2C2A" strokeWidth="0.8" strokeDasharray="4 5" opacity={0.38}/>
        </ThreadLeftG>
        <ThreadRightG id="home-thread-right" {...(animated && isNative ? { animatedProps: threadRightProps } : {})}>
